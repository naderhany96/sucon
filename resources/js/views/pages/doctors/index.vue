...
import EditSlot from "@/components/table/edit-slot";
...
export default {
    ...
    data() {
        return {
            ...
            items: [],
            // crud functions
            fetchLink: "doctors/list",
            deleteLink: "doctors/delete/",
            editRouteName: "Edit-Doctor",
            editSlotRouteName: "Edit-Slot",
            viewRouteName: "View-Doctor",
            GroupRouteName: "DoctorGroups",
            componentData: [],
            // table properties
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body dynamic_table_card_body">
                        ...
                        <div v-if="$can('add_doctor')">
                            <NavigateTo routeName="Add-Doctor" routeLabel="Add Doctor"></NavigateTo>
                            <hr>
                        </div>
                        ...
                        <div class="mb-0 position-relative dynamic_table">
                            <b-table primary-key="id" :tbody-transition-props="transProps" :items="componentData"
                                :fields="fields" :hover="hover" responsive="sm" :per-page="0" :current-page="currentPage"
                                :sort-desc.sync="sortDesc">
                                <template #cell(verified)="data">
                                    <div class="d-inline-block"
                                        v-if="data.item.email_verified_at == null || data.item.email_verified_at == ''">
                                ...
                                <template #cell(actions)="row">
                                    <div class="d-inline" v-if="$can('view_doctor')">
                                        <b-button size="sm" @click="viewItem(row.item, $event.target, viewRouteName)"
                                            class="mr-1" v-b-tooltip.hover title="View">
                                            <i class="fas fa-eye"></i>
                                        </b-button>
                                    </div>
                                    <EditItem :row="row" :routeName="editRouteName" :editItem="editItem"
                                        permissionName="edit_doctor"></EditItem>
                                    <EditSlot :row="row" :routeName="editSlotRouteName" :editItem="editItem"
                                        permissionName="edit_doctor"></EditSlot>
                                    <div class="d-inline" v-if="$can('view_doctor')">
                                        <b-button size="sm" @click="viewItem(row.item, $event.target, GroupRouteName)"
                                            class="mr-1" v-b-tooltip.hover title="Groups">
                                            <i class="fas fa-users"></i>
                                        </b-button>
                                    </div>
                                    <ConfirmDelete :row="row" :deleteItem="dynamicDeleteItem" :deleteLink="deleteLink"
                                        :dynamicFetchComponentData="dynamicFetchComponentData"
                                        permissionName="delete_doctor"></ConfirmDelete>
                                </template>
                            </b-table>
